      ...
      <div class="container-fluid">
        ...
        <div class="row">
          ...
          <!--card body-->
              ...
              JOIN
                  room_service ON orders.driver_id = room_service.driver_id where order_status='payment_approved' and room_service.email='$cemail'
            ") or die($mysqli->error);
              ...
              while ($row = $result->fetch_assoc()) {
                  echo "
                      ...
                      <td><a href='orders.php?apshpid=".$row['orderid']."' class='btn btn-success'>serviced</a></td>
                      </tbody>
                  ";
              }
              ?>
...
{
  $id=$_GET['apshpid'];
  $result=$mysqli->query("UPDATE orders set order_status='serviced' where id='$id'") or die($mysqli_error);
          ...
          <!--card body-->
                  <div class="class-body">
                   <div class="table-responsive ps">
                    <table id="example2" class="table table-responsive table-hover" id="page1">
                      <thead>
                        <tr><th>PROPERT NAME</th><th>DESCRIPTION</th><th>QUANTITY</th>
                        <th>CUSTOMER FIRSTNAME</th><th>CUSTOMER LAST NAME</th><th>CUSTOMERLOCATION</th><th>ROOM SERVICE_NAME</th>
                        <th>ROOM SERVICE_PHONENUMBER</th>
                        <th>ROOM STATUS</th></tr>
                      </thead>
                      <?php
                  $cemail=$_SESSION['username'];
                  $result = $mysqli->query("SELECT
              ...
              JOIN
                  customers ON customers.customer_id = orders.customer_id
              JOIN
                  room_service ON orders.driver_id = room_service.driver_id where orders.order_status='serviced' and room_service.email='$cemail'
            ") or die($mysqli->error);
